...
use super::app::{SmsWindow, SmsMessage};
...
fn view_new_chat_actions<'a>(app: &'a SmsWindow, spacing: &cosmic::cosmic_theme::Spacing) -> Element<'a, SmsMessage> {
...
fn view_contacts_list<'a>(app: &'a SmsWindow, spacing: &cosmic::cosmic_theme::Spacing) -> Element<'a, SmsMessage> {
...
fn view_conversations_list<'a>(app: &'a SmsWindow, spacing: &cosmic::cosmic_theme::Spacing) -> Element<'a, SmsMessage> {
    ...
    let mut filtered: Vec<_> = app.conversations
        .iter()
...
fn conversation_matches_search(app: &SmsWindow, conv: &Conversation) -> bool {
    ...
    conv.contact_name.to_lowercase().contains(&query)
        ...
        || normalize_phone_number(&conv.phone_number).contains(&normalize_phone_number(&app.search_query))
}
...
fn view_conversation_item<'a>(
    ...
    let display_name = get_contact_name(app, &conv.phone_number)
        .unwrap_or_else(|| conv.phone_number.clone());
...
fn view_thread_panel<'a>(app: &'a SmsWindow, spacing: &cosmic::cosmic_theme::Spacing) -> Element<'a, SmsMessage> {
    let Some(thread_id) = &app.selected_thread else {
        return widget::container(
            widget::text("Select a conversation to view messages")
                .size(14)
        )
        .width(Length::Fill)
        .height(Length::Fill)
    ...
    let Some(conv) = app.conversations.iter().find(|c| c.thread_id == *thread_id) else {
        return widget::container(
            widget::text("Conversation not found")
                .size(14)
        )
        .width(Length::Fill)
        .height(Length::Fill)
...
fn view_messages_list<'a>(app: &'a SmsWindow, spacing: &cosmic::cosmic_theme::Spacing) -> Element<'a, SmsMessage> {
...
fn view_message_bubble<'a>(
    ...
    if !is_sent {
        let phone_number = get_current_conversation_phone(app)
            .unwrap_or_else(|| msg.address.clone());
        
        let sender_label = get_contact_name(app, &phone_number)
...
fn view_message_input<'a>(app: &'a SmsWindow, spacing: &cosmic::cosmic_theme::Spacing) -> Element<'a, SmsMessage> {
    widget::row()
        .push(
